...
def predict(train, test, neighbors):
    ...
    for x in test.values.tolist():
        real.append(x[-1])
        label = knn_training.predict_classification(
            train=train, test_row=x, num_neighbors=neighbors
        )
...
def create_confusion_matrix(real, predicts):
    ...
    df_cfm = pd.DataFrame(
        confusion_matrix(real, predicts), index=list(set(real)), columns=list(set(real))
    )
